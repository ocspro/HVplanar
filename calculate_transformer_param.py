...
def transformer_inductance(layers, turns, radius_inner, h_dielec, pcb_core,
                           material):
    g = hvplanar.TransformerGeometry(layers_primary=layers,
    ...
    l, r = hvplanar.run_FEMM_inductance_prim(g)
...
def transformer_capacitance(layers, turns, radius_inner, h_dielec,
    ...
    g = hvplanar.TransformerGeometry(layers_primary=layers,
                                     ...
                                     radius_dielectric=radius_inner+50,
                                     material_dielectric=material)
    c = hvplanar.run_FEMM_capacitance(geometry=g)
...
def transformer_coupling(layers, max_turns, radius_inner, h_dielec, pcb_core,
    ...
    g = hvplanar.TransformerGeometry(layers_primary=layers,
                                     ...
                                     radius_dielectric=radius_inner+50,
                                     material_dielectric=material)
    l1, r1, l2, r2, m = hvplanar.run_FEMM_inductance_mutual(g)
    k = m / np.sqrt(l1 * l2)
    print ('k: {:.3f}'.format(k))
...
def transformer_all_parameters(layers, turns, radius_inner, h_dielec, pcb_core,
    ...
    l1, r1, l2, r2, m = hvplanar.run_FEMM_inductance_mutual(g)
    k = m / np.sqrt(l1 * l2)
    c = hvplanar.run_FEMM_capacitance(g)
...
def transformer_field(layers, turns, radius_inner, h_dielec,
    ...
    if gap:
        g.height_gap = gap
    C, field_peak = hvplanar.run_FEMM_field_distribution(g, voltage,
                                                         guard=guard)
    f_max_list = ', '.join(['{:.1f}'.format(f*1e-6) for f in field_peak])
    print ('C: {:.1f}'.format(C * 1e12 / voltage))
...
if __name__ == '__main__':
    ...
    if command == 1:
        ...
        res = transformer_field(layers, turns, radius_inner, d_height,
                                pcb_core_height, material, gap=gap,
                                guard=guard)
